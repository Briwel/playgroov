...
export function TrackStrip({ name, color, level, muted, solo, compact = false, onLevelChange, onToggleMute, onToggleSolo }: TrackStripProps) {
  const db = (level - 7) * 2;
  return (
    <View style={[styles.container, muted && styles.containerMuted]}>
      <View style={[styles.colorRail, { backgroundColor: color }]} />
      <View style={[styles.trackInfo, compact && styles.trackInfoCompact]}>
      ...
      <View style={styles.gainMeter} accessibilityLabel={`Niveau de ${name}: ${level} sur 12`}>
        {Array.from({ length: 12 }, (_, index) => (
          <View key={index} style={[styles.gainSegment, { backgroundColor: index < level ? color : '#383B35', opacity: muted ? 0.25 : 1 }]} />
        ))}
      </View>
...
const styles = StyleSheet.create({
  ...
  trackInfoCompact: { width: 91, gap: 6, paddingLeft: 7 },
  ...
  actionButton: { width: 27, height: 27, borderRadius: 6, borderWidth: 1, borderColor: theme.colors.ligne, alignItems: 'center', justifyContent: 'center', backgroundColor: '#20221F' },
